...
template.defaults.imports.signalFormat = function (num) {
    var n = num || 0;
    var str = Math.round(n / 20);
    return str;
};
template.defaults.imports.filterPlug = function (num) {
    var n = num || 0;
    var icon = 'icon-status-disabled';

    n = parseInt(n)
    switch (n) {
        case 1:
            icon = 'icon-status-used';
            break;
        case 2:
            icon = 'icon-status-notused';
            break;
    }
    return icon;
};
template.defaults.imports.dateFormat = function (date, formatStr) {
    /*[code..]*/
    let _date = date ? new Date(date * 1000) : new Date();
    let str = formatStr;
...
template.defaults.imports.secFormat = function (sec) {
    /*[code..]*/
    var _sec = parseInt(sec) || 0;
    ...
    var hours = parseInt(_sec / sechour),
        min = parseInt(((_sec % secday) % sechour) / 60),
        sec = ((_sec % secday) % sechour) % 60;
