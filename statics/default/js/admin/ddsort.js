    ...
    $.fn.DDSort = function (options) {
        var $doc = $(document),
            ...
            settings = $.extend(true, {
                ...
                move: fnEmpty,
                up: fnEmpty,
                top: 30,//消减的高度，根据实际情况来定
                target: 'li',
                ignore: '',//排除的样式
                skey: '',//指定的元素类型，只对此元素生效
                cloneStyle: {
        ...
        return this.each(function () {
            ...
            that.on('mousedown.DDSort', settings.target, function (e) {
                ...
                if (tagName == 'input' || tagName == 'textarea' || tagName == 'select' || tagName == 'button' || tagName == 'ins') {
                    return;
                }
                if (settings.skey && tagName != settings.skey) {
                    console.log('非指定类型', tagName);
                    return;
                }
                if (settings.ignore && $(e.target).hasClass(settings.ignore)) {
                    console.log('包含指定样式', settings.ignore);
                    return;
                }
