...
Calendar.prototype.draw = function () {
    ...
    var today = new Date(calendar.year, calendar.month, calendar.dateTime);
    if ((calendar.startDate && calendar.checkDate(calendar.startDate, today) > 0) || (calendar.endDate && calendar.checkDate(calendar.endDate, today) < 0)) {
        this.form.selectTodayButton.onclick = null;
        this.form.selectTodayButton.disabled = true
    } else {
...
Calendar.prototype.bindData = function () {
    ...
    for (var i = 0; i < tds.length; i++) {
        ...
        (tds[i].className == 'sun' || tds[i].className == 'sat') ? tds[i].style.color = '#ff2f2f' : tds[i].style.color = '#333';
        tds[i].onclick = null;
        tds[i].onmouseover = null;
        tds[i].onmouseout = null;
        tds[i].innerHTML = tval;
        if (i > dateArray.length - 1) continue;
        if (dateArray[i]) {
            if (tval != '&nbsp;') {
                var tdate = new Date(calendar.year, calendar.month, tval);
                if ((calendar.startDate && calendar.checkDate(calendar.startDate, tdate) > 0) || (calendar.endDate && calendar.checkDate(calendar.endDate, tdate) < 0)) {
                    tds[i].style.color = calendar.colors["bad"];
                    tds[i].onmouseover = null;
                    tds[i].onmouseout = null;
                    tds[i].onclick = null;
                    isOk = false
            ...
            if (isOk) {
                tds[i].onclick = function () {
                    if (calendar.dateControl) {
                        calendar.dateControl.value = new Date(calendar.year, calendar.month, this.innerHTML).format(calendar.date2StringPattern)
                    }
                    calendar.hide()
                }
                ...
                tds[i].onmouseout = function () {
                    ...
                    (this.className == 'sun' || this.className == 'sat') ? this.style.color = '#ff2f2f' : this.style.color = '#333'
                }
                if (today.getFullYear() == calendar.year && today.getMonth() == calendar.month && today.getDate() == dateArray[i]) {
...
Calendar.prototype.checkDate = function (date1, date2) {
    var stime = date1.getTime() - date2.getTime();
    if (stime < 0) {//date1>date2
        return -1;
    } else if (stime == 0) {//date1=date2
        return 0;
    } else {//date1<date2
        return 1;
    }
}
Calendar.prototype.setDateLimit = function (startDate, endDate) {
...
__ci.document.writeln('body {font-size:12px;margin:0px;text-align:center; color:#333;}');
__ci.document.writeln('form {margin:0px;}');
__ci.document.writeln('select {font-size:12px;background-color:#EFEFEF;}');
__ci.document.writeln('table {border:0px solid #CCCCCC;background-color:#FFFFFF;border-spacing:0px; border-style:none; border-collapse:collapse;}');
__ci.document.writeln('th {font-size:12px;font-weight:normal;background-color:#FFFFFF;}');
__ci.document.writeln('th.theader {font-weight:normal;background-color:#03bb9b;color:#FFFFFF;width:24px;}');
...
__ci.document.writeln('td.sat {color:#ff2f2f;background-color:#EFEFEF;}');
__ci.document.writeln('td.sun {color:#ff2f2f;background-color:#EFEFEF;}');
