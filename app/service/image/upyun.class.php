...
class UpYun {
     ...
     * 上传并转码为mp3
     * @param string $path 存储路径
     * @param mixed $file 需要上传的文件路径
     * @param string $notifyUrl 异步通知接口
     * @param array $opts 可选参数
     */

    public function changeMedia($path, $file, $notifyUrl = '', $opts = NULL) {
        $source = '/tmp_media/' . ltrim($file, '/');
        $res = $this->writeFile($source, file_get_contents($file), true, $opts);
        if ($res) {
            return $this->saveMedia($source, $path, $notifyUrl);
        } else {
            return '';
        }
    }

    public function saveMedia($source, $path, $notifyUrl) {
        T('image/AvPretreatment');
        $avPretreatment = new AvPretreatment($this->_username, $this->orgPassWord);
        $task = json_encode(array(array(
                'name' => 'naga',
                'type' => 'audio',
                'save_as' => $path,
                'avopts' => '/f/mp3/ab/92/',
        ...
        $res = $avPretreatment->request($data);
        if ($res) {
            return isset($res[0]) ? $res[0] : '';
        } else {
            $this->error = $avPretreatment->getErrMsg();
            return '';
        }
    }
    ...
    protected function _do_request($method, $path, $headers = NULL, $body = NULL, $file_handle = NULL) {/* {{{ */
        ...
        if ($http_code == 0) {
            $this->error = $http_code . ':Connection Failed';
            curl_close($ch);
            return '';
        }
