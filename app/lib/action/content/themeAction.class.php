...
class themeAction extends commonAction
{
    ...
    public function post()
    {
        ...
        $insertdata['login_ip'] = ip2long($_SERVER['REMOTE_ADDR']);
        $insertdata['effective'] = $validity_time ? strtotime($validity_time . '00:00:00') : 0;
        $insertdata['city_id'] = $city_id;
        $insertdata['group_id'] = $group_id;
        $insertdata['status'] = $status;
        $insertdata['psw'] = D('admin')->setUserPassword($pwd);
        $insertdata['create_time'] = time();
        $user = M('admin')->where(array('username' => $username, 'status' => array(1, 2)))->find();
        if ($user) {
